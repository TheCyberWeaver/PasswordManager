...
class PasswordManagerWindow(QWidget):
    ...
    def updateStatistic(self, result):
        ...
        f = open(os.path.join(os.path.abspath(os.path.dirname(__file__)),"Resources/All_List.txt"), 'r')
        ...
        strength_percentage = int(strength / (4 * len(result)) * 100) if len(result)>0 else 0
        self.circular_progress_1.set_value(len(result))
        self.circular_progress_2.set_value(strength_percentage)
        # print(leakedcount)
        leaked_percentage= int(leakedcount / len(result) * 100) if len(result)>0 else 0
        self.circular_progress_3.set_value(leaked_percentage)
